...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        get("/Home", (request, response) -> {
            ...
            attributes.put("Estudiantes", DB.getInstance().getEstudiantes().values());
            return new ModelAndView(attributes, "Home.ftl");
        }, freeMarkerEngine);

        get("/Visualizar", (request, response) -> {
            Map<String, Object> attributes = new HashMap<>();
            attributes.put("Estudiante", DB.getInstance().getEstudiantes().get(Integer.parseInt(request.queryParams("Mat").replace(",",""))));
        ...
        post("/Registro", (request, response) -> {
            ...
            if(!DB.getInstance().getEstudiantes().containsKey(est.getMatricula())) {
                System.out.println("insert");
                DB.getInstance().getEstudiantes().put(est.getMatricula(), est);
            }

            System.out.println(DB.getInstance().getEstudiantes().size());
        ...
        post("/Borrar", (request, response) -> {
            ...
            if(DB.getInstance().getEstudiantes().containsKey(mat)) {
                System.out.println("delete");
                DB.getInstance().getEstudiantes().remove(mat);
            }
        ...
        post("/Modificar", (request, response) -> {
            ...
            if(mat0 == mat && DB.getInstance().getEstudiantes().containsKey(mat0)){
                Estudiante est = new Estudiante(mat,request.queryParams("Nom"),request.queryParams("Ape"),request.queryParams("Tel"));
                DB.getInstance().getEstudiantes().put(est.getMatricula(),est);
            }else if(DB.getInstance().getEstudiantes().containsKey(mat0) && !DB.getInstance().getEstudiantes().containsKey(mat)) {
                System.out.println("delete in modify");
                DB.getInstance().getEstudiantes().remove(mat0);
                Estudiante est = new Estudiante(mat,request.queryParams("Nom"),request.queryParams("Ape"),request.queryParams("Tel"));
                DB.getInstance().getEstudiantes().put(est.getMatricula(),est);
            }

            System.out.println("modificar"+DB.getInstance().getEstudiantes().size());
